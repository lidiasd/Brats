...
À l'époque 11 sur 15, les résultats sur l'ensemble de test montrent une performance globale modérée du modèle. La métrique Mean IoU (Intersection over Union) est de 39,31 %. Le dice coefficient, qui mesure la similarité entre les prédictions et les vérités terrain, est élevé à 61,23 %. Les valeurs de précision, sensibilité et spécificité sont également élevées, démontrant la capacité du modèle à classifier les différents types de tumeurs mais globalement le modèle reste inférieur à la moyenne. En ce qui concerne les sous-régions tumorales, les coefficients de Dice pour la nécrose, l'œdème et la tumeur active sont respectivement de 58,46 %, 69,64 % et 72,15 %, avec des valeurs plus élevées pour l'œdème et la tumeur active, ce qui suggère une segmentation précise de ces régions.
